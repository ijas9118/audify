document.addEventListener('DOMContentLoaded', () => {
  ...
  const updateSummary = (summary) => {
    document.querySelector('#totalSalesCount').textContent = `${summary.totalSalesCount} Orders`;
    document.querySelector('#overallOrderAmount').textContent = `₹${summary.overallOrderAmount.toFixed(2)}`;
    document.querySelector('#overallDiscount').textContent = `₹${summary.overallDiscount.toFixed(2)}`;
  ...
  pdfButton.addEventListener('click', () => {
    ...
    // Table headers
    const tableHeaders = ['Date', 'Total Sales Revenue', 'Discount Applied', 'Net Sales', 'Number of Orders', 'Total Items Sold'];
    ...
    doc.text(`Overall Order Amount: ${summaryData.overallOrderAmount.toFixed(2)}`, 10, finalY + 30);
    doc.text(`Overall Discount: ${summaryData.overallDiscount.toFixed(2)}`, 10, finalY + 40);
